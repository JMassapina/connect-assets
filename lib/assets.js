...
(function() {
  var BEFORE_DOT, Browserify, ConnectAssets, EXPLICIT_PATH, REMOTE_PATH, Snockets, connectCache, crypto, cssCompilers, cssExts, exports, fs, getExt, jsCompilers, libs, md5Filenamer, mkdirRecursive, parse, path, stripExt, timeEq, _;
  ...
  module.exports = exports = function(options) {
    ...
    if ((_ref12 = options.browserify) == null) {
  ...
  ConnectAssets = (function() {
    ...
    function ConnectAssets(options) {
      ...
      this.cache.middleware.connectAssets = this;
      this.snockets = new Snockets({
        src: this.options.src
      });
      this.jsPackager = this.options.browserify ? new Browserify({
        src: this.options.src,
        buildFilenamer: this.options.buildFilenamer,
        browserifyCache: this.options.browserifyCache
      }) : this.snockets;
      this.cssSourceFiles = {};
      this.compiledCss = {};
      this.buildFilenames = {};
    ...
    ConnectAssets.prototype.createHelpers = function() {
      ...
      context.css = function(route, routeOptions) {
        ...
        if ((routeOptions != null ? routeOptions.media : void 0) != null) {
          tag.push(" media='" + routeOptions.media + "'");
        }
        tag.push('>');
        return tag.join('');
      };
    ...
    ConnectAssets.prototype.compileJS = function(route) {
      var callback, chain, ext, filename, js, options, sourcePath, _i, _len, _ref,
        _this = this;
      if (!this.options.detectChanges && this.cachedRoutePaths[route]) {
        return this.cachedRoutePaths[route];
      }
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        try {
          if (this.options.build) {
            ...
            options = {
              debug: true,
              watch: this.options.detectChanges,
              minify: this.options.minifyBuilds
            };
            this.jsPackager.getConcatenation(sourcePath, options, callback);
            return this.cachedRoutePaths[route] = ["/" + filename];
          } else {
            chain = this.jsPackager.getCompiledChain(sourcePath, {
              async: false
            });
  ...
  exports.cssCompilers = cssCompilers = {
    styl: {
      ...
      compileSync: function(sourcePath, source, helperContext) {
        var callback, img, options, result, _base, _ref;
        result = '';
        ...
        img = function(node) {
          ...
          return new libs.stylus.nodes.String(imgPath);
        };
        libs.stylus(source, options).use(libs.bootstrap()).use(libs.nib()).use(libs.bootstrap()).set('compress', this.compress).set('include css', true).define('img', img).render(callback);
        return result;
      }
    },
    less: {
